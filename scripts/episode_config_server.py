...
SPAWN_YAW_RANGE   = (0.0, 2 * math.pi)   # full rotation around Z — boxes have 4-fold symmetry anyway
SPAWN_SCALE_RANGE = (0.90, 1.10)          # ±10% uniform size variation
...
def sample_episode(all_objects, bin_mapping, bin_positions, n_objects, rng):
    ...
    for obj, (x, y, z) in zip(active, positions):
        spawned.append({
            ...
            "x": x, "y": y, "z": z,
            "yaw":   rng.uniform(*SPAWN_YAW_RANGE),
            "scale": rng.uniform(*SPAWN_SCALE_RANGE),
        })
...
def run(sim_config_path, n_objects):
    ...
    sock.settimeout(1.0)   # allow Ctrl+C to be processed between recvfrom calls
    ...
    try:
        while True:
            ...
            try:
                ...
                mp = lt["main_pos"]
                log.info(
                    "  lighting | main_pos=(%.2f,%.2f,%.2f) main_diffuse=(%.2f,%.2f,%.2f) fill=%.2f",
                    mp[0], mp[1], mp[2],
